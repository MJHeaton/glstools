...
#' Function to *correctly* predict values from a GLS model by accounting for
#' variance and correlation structures.
#'
#' @param glsobj A fitted GLS model object.
#' @param newdframe A dataframe of new values to predict.
#' @param level Confidence level for prediction interval.
#'
#' @return Dataframe of test data with prediction intervals and predictive SE.
#' @export
#'
#' @examples
...
predictgls <- function(glsobj, newdframe = NULL, level = 0.95) {
  ...
  if (is.null(newdframe)) {
    warning(paste(
      "No newdframe provided for prediction.",
  ...
  if ("varStruct" %in% names(glsobj$modelStruct)) {
    ...
    if (length(var.pars) == 0) {
      ...
      var.call <- deparse(glsobj$call$weights)
      var.call <- paste(
        substr(var.call, 1, nchar(var.call) - 1), ", value = c(",
  ...
  if ("corStruct" %in% names(glsobj$modelStruct)) {
    ...
    cor.call <- paste(
      substr(cor.call, 1, nchar(cor.call) - 1), ", value = c(",
    ...
    if (gsub(".*:", "", as.character(glsobj$call$correlation)[1]) == "corSymm") {
      warning(paste(
        "The general correlation structure corSymm() cannot be used",
      ...
      warning(paste(
        "Some prediction locations are the same as observed locations",
